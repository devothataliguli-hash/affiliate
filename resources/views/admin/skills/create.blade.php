...
<div class="max-w-7xl mx-auto px-3 sm:px-4">
    ...
    <div class="rounded-xl shadow-md overflow-hidden" style="background-color: #FFFFFF;">
        <div class="px-5 md:px-6 py-4 border-b" style="background: linear-gradient(135deg, #FFF3E0, #FFE0B2); border-color: #E5E7EB;">
            <h1 class="text-lg md:text-xl font-bold" style="color: #1F2937;">Add New Skill</h1>
            <p class="text-sm mt-0.5" style="color: #6B7280;">Create a new learning skill for your students</p>
        </div>
        
        <form action="{{ route('admin.skills.store') }}" method="POST" class="p-5 md:p-6">
            @csrf
            ...
            <div class="mb-5">
                ...
                <textarea name="description" rows="4" class="w-full px-4 py-3 rounded-lg border transition-all focus:outline-none focus:ring-2" style="border-color: #D1D5DB; background-color: #FFFFFF; color: #1F2937;">{{ old('description') }}</textarea>
                <p class="text-xs mt-1" style="color: #6B7280;">Brief description of what students will learn</p>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-5">
                <div>
                    ...
                    <input type="number" name="price" class="w-full px-4 py-3 rounded-lg border transition-all focus:outline-none focus:ring-2" style="border-color: #D1D5DB; background-color: #FFFFFF; color: #1F2937;" value="{{ old('price', 0) }}" required>
                    <p class="text-xs mt-1" style="color: #6B7280;">Set 0 for free skills</p>
                ...
                <div>
                    ...
                    <input type="text" name="icon" class="w-full px-4 py-3 rounded-lg border transition-all focus:outline-none focus:ring-2" style="border-color: #D1D5DB; background-color: #FFFFFF; color: #1F2937;" value="{{ old('icon', 'fa-graduation-cap') }}" placeholder="fa-chart-line">
                    <p class="text-xs mt-1" style="color: #6B7280;">Use Font Awesome 6 icons (e.g., fa-chart-line, fa-store, fa-users)</p>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-5">
                <div>
                    ...
                    <div class="flex items-center gap-3">
                        <input type="color" name="color" class="h-10 w-16 border rounded-lg cursor-pointer" style="border-color: #D1D5DB;" value="{{ old('color', '#FF6F00') }}">
            ...
            <div class="mb-6">
                <label class="flex items-center gap-3 cursor-pointer">
                    <input type="checkbox" name="is_active" value="1" class="w-4 h-4 rounded" style="accent-color: #F57C00;" {{ old('is_active', true) ? 'checked' : '' }}>
                    <span class="text-sm font-semibold" style="color: #374151;">Active (visible to users)</span>
                </label>
            </div>
            
            <div class="flex flex-wrap gap-3">
                <button type="submit" class="font-semibold px-6 py-2.5 rounded-lg transition hover:shadow-md active:scale-98" style="background-color: #F57C00; color: #FFFFFF;">Create Skill</button>
